...
import App from "./test-auth";
import { PaymentItems, SubscriptionItems } from "@dodopayments/better-auth";
...
function CardExample() {
  ...
  const [subscription, setSubscription] =
    React.useState<SubscriptionItems | null>(null);
  const [payments, setPayments] = React.useState<PaymentItems | null>(null);
  const getPaymentsandSubscription = async () => {
    try {
      ...
      console.error("Error fetching subscriptions and payments:", error);
    }
  };
        ...
        <CardContent>
          {subscription && payments ? (
            <div>
              ...
              <pre className="mb-4 max-h-40 overflow-auto rounded  p-2 text-sm">
                {JSON.stringify(subscription, null, 2)}
              </pre>
              <h3 className="mb-2 text-lg font-semibold">Payments:</h3>
              <pre className="max-h-40 overflow-auto rounded  p-2 text-sm">
            ...
            <p className="mb-4 text-sm text-gray-600">
              {session
                ? "Fetching subscription and payment details..."
                : "Sign in to view your subscription and payment details."}
            </p>
          )}
          {session ? (
            <>
